    ...
    <div className={cn('flex gap-2 sm:gap-3', className)} role="group" aria-label={`${length}-digit code`}>
      {Array.from({ length }).map((_, index) => (
        <input
          ...
          className="h-14 w-full min-w-0 rounded-field border border-line bg-canvas text-center font-display text-xl font-bold text-aubergine outline-none transition-colors placeholder:text-ink-40 focus:border-line-strong focus:shadow-ring disabled:opacity-60"
        />
      ))}
    </div>
  );
