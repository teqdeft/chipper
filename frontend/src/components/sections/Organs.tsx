          ...
          <motion.div key={organ.name} variants={revealItem}>
            <article className="card card-hover group flex items-center justify-between gap-3 bg-canvas p-5 sm:gap-4 sm:p-6">
              <div className="flex min-w-0 items-center gap-4 sm:gap-5">
                <span className="font-display text-sm font-bold text-ink-40 tabular-nums">
                  {String(i + 1).padStart(2, '0')}
                </span>
                <div className="min-w-0">
                  <h3 className="font-display text-lg font-bold text-aubergine sm:text-xl">{organ.name}</h3>
                  <p className="mt-1 text-sm leading-snug text-ink-55">{organ.note}</p>
                </div>
              </div>
              <span
                className="text-ink-40 transition-all duration-300 ease-premium group-hover:translate-x-1 group-hover:text-coral"
                aria-hidden
              >
                →
              </span>
